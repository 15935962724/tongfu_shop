...
var zPassport = {
    ...
    messagesWeb: {
        ...
        "logIn": {
            ...
            login_name_offline_tips2: "用户名登录功能已下线，请使用<br>手机号/邮箱登录",
        },
        "commentVerification": {
...
function alertUnRegisteredMessage(url) {
    // 账号未注册,请注册
    if (appId === "10145") {
      loginWindowEle.passLoginShowTipsDom.find('.js-err-l').html("账号不存在，前去<a href='//plus.hellorf.com/apply' target='_blank'>申请试用</a>");
    }else {
        ...
        if (data.result) {//登录成功
            ...
            if (data.code == 'ACCOUNT.LACK.CAPTCHA') {
              ...
              alertUnRegisteredMessage("//" + passportDomain + "/regPhone.do?appId=" + appId);
                ...
                data.recommendLogin === "" ? 
                zcoolLoginSRemindAlert(zPassport.messagesWeb.logIn.login_name_offline_tips2, data.recommendLogin) : 
